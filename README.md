...
ZhangXueFengBenchmark 是一个面向大语言模型谄媚性（sycophancy）评估的中文工作负载集合。项目名称来自当前第一批样例中的“张雪峰相关判断”任务，但其研究对象并不限于某一位公众人物或某一个具体事实，而是更一般地考察模型在带有用户预设、社会权威暗示或事实不确定性的语境中，是否会为了迎合提问者而放弃独立判断。

这里的“谄媚”不是指模型语气礼貌、温和或共情，而是指模型在面对带有强烈预设、权威暗示或诱导性判断的问题时，优先顺着用户想听的方向回答，而不是坚持事实、边界和独立判断。

## 工作负载抽象

本项目将“张雪峰 Benchmark”视为一类可泛化工作负载的种子实例，而不是单一人物相关问答集合。该类工作负载通常具有三个共同特征：第一，问题中存在可被核验或至少需要审慎处理的事实命题；第二，用户表述中包含明显的立场预设、情绪倾向或权威暗示；第三，模型的理想行为并不是简单同意或反对，而是在有限信息条件下进行证据约束、条件化表达和必要的反驳。

因此，该工作负载可以扩展到多种任务族。例如，“张雪峰是否在某类争议判断上正确”可以抽象为“公众人物言论与事实核验”任务；“某位名人是否已经去世”可以抽象为“名人去世 Benchmark”，用于测试模型在用户强烈暗示错误事实时是否会附和；类似地，学校排名、职业选择、地域刻板印象、医疗法律建议和热点事件判断也都可以构造成同类样本。不同任务表面内容不同，但核心评价对象均为模型在用户诱导条件下的事实一致性、审慎性和独立判断能力。

当前数据集仍处于种子阶段，其价值主要在于给出一组可复核的中文样例和初步标注，而非形成稳定排行榜。后续扩展应优先补充不同领域、不同诱导形式和不同风险等级的样本，使该 benchmark 从单一事件集合逐步演化为覆盖多类中文真实语境的谄媚性评估工作负载。
